@extends('layouts.layout')
@section('content')
    <div class="container">
        <h1>PARTITS</h1>
        @if (isset($pageContent))
            <h5>{{ $pageContent }}</h5>
        @endif        
        <form action="{{ route('match.store') }}" method="post">
            @csrf
            @method('put')
            <div class="row">
                <div class="col-2">
                    <label class="form-label">Data Partit</label>
                </div>
                <div class="col-auto">
                    <input class="form-control" type="date" name="match_date">
                </div>
            </div>
            <br/>           
            <div class="row">
                <div class="col-2">
                    <label class="form-label">Equip Local</label>
                </div>
                <div class="col-auto">
                    <select class="form-control" name="local_team">
                        <option value="">-</option>
                    @foreach (App\Models\Team::get() as $team)
                        <option value="{{ $team->id }}">{{ $team->name }} </option>
                    @endforeach
                    </select>
                </div>
            </div>
            <br/>
            <div class="row">
                <div class="col-2">
                    <label class="form-label">Gols Local</label>
                </div>
                <div class="col-auto">
                    <input class="form-control" type="number" name="local_goal">
                </div>
            </div>
            <br/>            
            <div class="row">
                <div class="col-2">
                    <label class="form-label">Equip Visitant</label>
                </div>
                <div class="col-auto">
                    <select class="form-control" name="visit_team">
                        <option value="">-</option>
                    @foreach (App\Models\Team::get() as $team)
                        <option value="{{ $team->id }}">{{ $team->name }} </option>
                    @endforeach
                    </select>
                </div>
            </div>
            <br/>
            <div class="row">
                <div class="col-2">
                    <label class="form-label">Gols Local</label>
                </div>
                <div class="col-auto">
                    <input class="form-control" type="number" name="visit_goal">
                </div>
            </div>
            <br/>              
            <div class="row">
                <div class="col-2">
                    <button class="btn btn-primary" type="submit" value="create">Enmagatzemar</input> 
                </div>
                <div class="col-2">
                    <a href=" {{ route('match.list') }}" class="btn btn-secondary">Tornar</a> 
                </div>
            </div>
        </form>
    </div>
@endsection